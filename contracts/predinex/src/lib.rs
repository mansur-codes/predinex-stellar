...
pub enum DataKey {
    ...
    Webhooks,
    /// #420 — Referral fee in basis points (0 = disabled). Set by treasury recipient.
    ReferralBps,
    /// #420 — Pending referral reward balance per referrer address.
    ReferralBalance(Address),
    /// #420 — Contract-wide cumulative referral volume (sum of all referred bet amounts).
    TotalReferralVolume,
}
...
pub enum ContractError {
    ...
    WebhookNotFound = 54,
    /// #420 — Referrer and bettor are the same address.
    SelfReferral = 55,
    /// #420 — No referral rewards available to claim.
    NoReferralRewards = 56,
}
